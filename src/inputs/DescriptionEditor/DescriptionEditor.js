...
export const DescriptionEditor = (props) => {
  const { onChange, readOnly, value, scenarioId, labels } = props;
  ...
  const description = useMemo(() => {
    if (readOnly || !isEditing)
      return (
        ...
        >
          <Typography
            data-cy={`${scenarioId}-description-disabled`}
            sx={{
              maxHeight: '75px',
              overflowY: 'auto',
            }}
    ...
    return (
      <TextField
        data-cy={`${scenarioId}-description`}
        variant="outlined"
        fullWidth={true}
        multiline
        rows={3}
  ...
  }, [isEditing, readOnly, descriptionText, value, startEdition, stopEdition, scenarioId, labels.placeholder]);
...
DescriptionEditor.propTypes = {
  /**
   * Callback function called when description edited. This function takes a single parameter:
  ...
  /**
   * Optional scenario id. Can be useful to identify the component and customize data-cy metadata.
   */
